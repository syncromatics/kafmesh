...
type OutputSliceLoader struct {
	...
	cache map[int][]*model.ProcessorOutput

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *outputSliceLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type outputSliceLoaderBatch struct {
	keys    []int
	data    [][]*model.ProcessorOutput
	error   []error
	closing bool
	done    chan struct{}
}
...
func (l *OutputSliceLoader) LoadThunk(key int) func() ([]*model.ProcessorOutput, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.ProcessorOutput, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &outputSliceLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
	...
	return func() ([]*model.ProcessorOutput, error) {
		...
		var data []*model.ProcessorOutput
...
func (l *OutputSliceLoader) LoadAll(keys []int) ([][]*model.ProcessorOutput, []error) {
	...
	processorOutputs := make([][]*model.ProcessorOutput, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		processorOutputs[i], errors[i] = thunk()
	}
	return processorOutputs, errors
}
...
func (l *OutputSliceLoader) Prime(key int, value []*model.ProcessorOutput) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.ProcessorOutput, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
