...
type LookupSliceLoader struct {
	...
	cache map[int][]*model.ProcessorLookup

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *lookupSliceLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type lookupSliceLoaderBatch struct {
	keys    []int
	data    [][]*model.ProcessorLookup
	error   []error
	closing bool
	done    chan struct{}
}
...
func (l *LookupSliceLoader) LoadThunk(key int) func() ([]*model.ProcessorLookup, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.ProcessorLookup, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &lookupSliceLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
	...
	return func() ([]*model.ProcessorLookup, error) {
		...
		var data []*model.ProcessorLookup
...
func (l *LookupSliceLoader) LoadAll(keys []int) ([][]*model.ProcessorLookup, []error) {
	...
	processorLookups := make([][]*model.ProcessorLookup, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		processorLookups[i], errors[i] = thunk()
	}
	return processorLookups, errors
}
...
func (l *LookupSliceLoader) Prime(key int, value []*model.ProcessorLookup) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.ProcessorLookup, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
