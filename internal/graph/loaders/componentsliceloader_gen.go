...
type ComponentSliceLoader struct {
	...
	cache map[int][]*model.Component

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *componentSliceLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type componentSliceLoaderBatch struct {
	keys    []int
...
func (l *ComponentSliceLoader) LoadThunk(key int) func() ([]*model.Component, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.Component, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &componentSliceLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *ComponentSliceLoader) LoadAll(keys []int) ([][]*model.Component, []error) {
	...
	components := make([][]*model.Component, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		components[i], errors[i] = thunk()
	}
...
func (l *ComponentSliceLoader) Prime(key int, value []*model.Component) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.Component, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
