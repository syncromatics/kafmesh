...
type viewSourceSliceLoader struct {
	...
	cache map[int][]*model.ViewSource

	// the current batch. keys will continue to be collected until timeout is hit,
	// then everything will be sent to the fetch method and out to the listeners
	batch *viewSourceSliceLoaderBatch

	// mutex to prevent races
	mu sync.Mutex
}

type viewSourceSliceLoaderBatch struct {
	keys    []int
	data    [][]*model.ViewSource
	error   []error
	closing bool
	done    chan struct{}
}
...
func (l *viewSourceSliceLoader) LoadThunk(key int) func() ([]*model.ViewSource, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*model.ViewSource, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &viewSourceSliceLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *viewSourceSliceLoader) LoadAll(keys []int) ([][]*model.ViewSource, []error) {
	...
	viewSources := make([][]*model.ViewSource, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		viewSources[i], errors[i] = thunk()
	}
	return viewSources, errors
}
...
func (l *viewSourceSliceLoader) Prime(key int, value []*model.ViewSource) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*model.ViewSource, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
